...
if((a.paddle&&b.ball)||(a.ball&&b.paddle)||(a.soldier&&b.soldier&&a.controller!=b.controller)||(a.soldier&&b.paddle&&a.controller!=b.controller)||(a.paddle&&b.soldier&&a.controller!=b.controller)||(a.miner&&b.scorebox&&a.controller!=b.controller)||(a.scorebox&&b.miner&&a.controller!=b.controller)){ig.Entity.solveCollision(a,b);}};ig.Entity.solveCollision=function(a,b){var weak=null;if(a.collides==ig.Entity.COLLIDES.LITE||b.collides==ig.Entity.COLLIDES.FIXED){weak=a;}
...
ig.baked=true;ig.module('game.levels.level').requires('impact.image').defines(function(){LevelLevel={"entities":[],"layer":[]};});

// lib/game/entities/paddlebrick.js
ig.baked=true;ig.module('game.entities.paddlebrick').requires('impact.entity').defines(function(){EntityPaddlebrick=ig.Entity.extend({size:{x:8,y:8},collides:ig.Entity.COLLIDES.FIXED,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/sprites/square.png',8,8),health:50,maxVel:{x:0,y:200},velocity:128,controller:-1,paddle:true,init:function(x,y,settings){this.parent(x,y,settings);}});});

// lib/game/entities/playerbrick.js
ig.baked=true;ig.module('game.entities.playerbrick').requires('game.entities.paddlebrick').defines(function(){EntityPlayerbrick=EntityPaddlebrick.extend({init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[this.controller]);},update:function(){if(ig.input.state(this.input.up)){this.vel.y=-this.velocity;}else if(ig.input.state(this.input.down)){this.vel.y=this.velocity;}else{this.vel.y=0;}
this.parent();}});});

// lib/game/audio.js
ig.baked=true;ig.module('game.audio').defines(function(){SOUNDS={bounceX:new ig.Sound('media/sounds/bounceX.*'),bounceY:new ig.Sound('media/sounds/bounceY.*'),bounceZ:new ig.Sound('media/sounds/bounceZ.*'),hit:new ig.Sound('media/sounds/hit.*'),};bounceArr=[SOUNDS.bounceZ,SOUNDS.bounceY];bounceSoundCounter=0;bounceSoundTimer=new ig.Timer(.1);playNextBounce=function(){if(bounceSoundTimer.delta()>=0){bounceSoundTimer.reset();bounceArr[bounceSoundCounter].play();bounceSoundCounter=(bounceSoundCounter+1)%2;}};});

// lib/game/entities/ball.js
ig.baked=true;ig.module('game.entities.ball').requires('impact.entity','game.audio').defines(function(){EntityBall=ig.Entity.extend({size:{x:8,y:8},collides:ig.Entity.COLLIDES.LITE,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,animSheet:new ig.AnimationSheet('media/sprites/square.png',8,8),velSqr:256*256,maxVel:{x:256,y:256},vel:{x:182,y:182},currentVel:{x:0,y:0},ball:true,opponent:-1,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('white',1,[0]);this.addAnim('pink',1,[1]);},update:function(){this.currentVel={x:this.vel.x,y:this.vel.y};if(this.pos.y+this.size.y>=ig.system.height){this.pos.y=ig.system.height-8;this.vel.y=-this.vel.y;playNextBounce();}else if(this.pos.y<=0){this.vel.y=-this.vel.y;this.pos.y=0;playNextBounce();}
if(this.pos.x+this.size.x>=ig.system.width){this.vel.x=-this.vel.x;this.pos.x=ig.system.width-8;ig.game.score[0]++;playNextBounce();}else if(this.pos.x<=0){this.vel.x=-this.vel.x;this.pos.x=0;ig.game.score[1]++;playNextBounce();}
this.parent();},collideWith:function(other,axis){if(axis=="x"){var i=this.pos.x<ig.system.width/2?0:1;var mul=Math.abs(this.pos.y-ig.game.centers[i])/48;this.vel.y=(this.currentVel.y>0?1:-1)*Math.round(256*mul);if(Math.abs(this.vel.y)>222){this.vel.y=(this.vel.y>0?1:-1)*222;}
this.vel.x=(this.currentVel.x>0?-1:1)*Math.sqrt(this.velSqr-this.vel.y*this.vel.y);}else{this.vel.y=-this.currentVel.y;}
this.controller=other.controller;this.opponent=(this.controller+1)%2;if(other.controller==0){this.currentAnim=this.anims.white;}else{this.currentAnim=this.anims.pink;}
playNextBounce();},check:function(other){if(other.soldier&&other.controller==this.opponent){other.kill();SOUNDS.hit.play();}}});});

// lib/game/entities/soldier.js
ig.baked=true;ig.module('game.entities.soldier').requires('impact.entity').defines(function(){EntitySoldier=ig.Entity.extend({size:{x:8,y:8},collides:ig.Entity.COLLIDES.FIXED,type:ig.Entity.TYPE.A,controller:-1,velocity:48,target:null,curAngle:0,state:"walk",soldier:true,health:50,collided:false,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[this.controller*4]);this.addAnim('walk',.1,[this.controller*4+1,this.controller*4+2]);this.addAnim('attack',.2,[this.controller*4,this.controller*4+3]);this.currentAnim=this.anims.walk;if(this.controller==1){this.anims.idle.flip.x=true;this.anims.walk.flip.x=true;this.anims.attack.flip.x=true;}
this.opponent=(this.controller+1)%2;},chooseTarget:function(){bricks=ig.game.getEntitiesByType(EntityPaddlebrick);enemies=[];for(var i=0;i<bricks.length;i++){if(bricks[i].controller==this.opponent){enemies.push(bricks[i]);}}
this.target=enemies[Math.floor((Math.random()*enemies.length))];},update:function(){this.collided=false;if(this.pos.y+this.size.y>=ig.system.height){this.pos.y=ig.system.height-8;this.vel.y=-this.vel.y;}else if(this.pos.y<=0){this.vel.y=-this.vel.y;this.pos.y=0;}
this.parent();},setVelocity:function(velocity){velocity=typeof velocity!=='undefined'?velocity:this.velocity;this.vel.x=Math.ceil(Math.cos(this.curAngle)*velocity);this.vel.y=Math.ceil(Math.sin(this.curAngle)*velocity);},normalizeAngle:function(rad){rad=rad%(2*Math.PI);return rad>=0?rad:rad+2*Math.PI;},walkToTarget:function(velocity){velocity=typeof velocity!=='undefined'?velocity:this.velocity;this.curAngle=this.angleTo(this.target);this.setVelocity(velocity);},collideWith:function(other,axis){this.collided=true;if(axis=="y"&&other.paddle&&other.vel.y!=0){this.receiveDamage(1,other);}else{if(this.state=="walk"){if((other.paddle||other.soldier||other.scorebox)&&other.controller==this.opponent){this.target=other;this.state="attack";this.currentAnim=this.anims.attack;this.walkToTarget(4);}}else if(this.state=="attack"){if((other.paddle||other.soldier||other.scorebox)&&other.controller==this.opponent){this.target=other;other.receiveDamage(1,this);if(other._killed){this.chooseTarget();this.state="walk";this.currentAnim=this.anims.walk;}else{this.walkToTarget(4);}}}}
this.parent();}});});

// lib/game/entities/swordsman.js
ig.baked=true;ig.module('game.entities.swordsman').requires('game.entities.soldier').defines(function(){EntitySwordsman=EntitySoldier.extend({animSheet:new ig.AnimationSheet('media/sprites/sword.png',8,8),swordsman:true,passedOuter:false,init:function(x,y,settings){this.parent(x,y,settings);this.personality=0;this.chooseTarget();},update:function(){if(this.target.health==0){this.chooseTarget();this.state="walk";this.currentAnim=this.anims.walk;}
if(this.state=="walk"){this.curAngle=this.normalizeAngle(this.angleTo(this.target));this.setVelocity();}else if(this.state=="attack"){if(!this.collided){this.state="walk";this.currentAnim=this.anims.walk;}}
if(this.controller==0){if(this.pos.x+8>ig.game.outer[1]&&!this.passedOuter){if(Math.abs(this.pos.y-ig.game.centers[1])<36){this.passedOuter=true;}else{this.pos.x=ig.game.outer[1]-8;}}}else if(this.controller==1){if(this.pos.x<ig.game.outer[0]&&!this.passedOuter){if(Math.abs(this.pos.y-ig.game.centers[0])<36){this.passedOuter=true;}else{this.pos.x=ig.game.outer[0];}}}
this.parent();},collideWith:function(other,axis){this.parent(other,axis);}});});

// lib/game/entities/pikeman.js
ig.baked=true;ig.module('game.entities.pikeman').requires('game.entities.soldier').defines(function(){EntityPikeman=EntitySoldier.extend({animSheet:new ig.AnimationSheet('media/sprites/pike.png',8,8),pikeman:true,health:75,state:"defend",init:function(x,y,settings){this.parent(x,y,settings);this.chooseTarget();},chooseTarget:function(){soldiers=ig.game.getEntitiesByType(EntitySoldier);enemies=[];for(var i=0;i<soldiers.length;i++){if(soldiers[i].controller==this.opponent){if((this.controller==0&&soldiers[i].pos.x<ig.system.width/2)||(this.controller==1&&soldiers[i].pos.x>ig.system.width/2)){enemies.push(soldiers[i]);}}}
if(enemies.length>0){this.target=enemies[Math.floor((Math.random()*enemies.length))];this.state="walk";this.currentAnim=this.anims.walk;}else{this.target=null;this.state="defend";this.currentAnim=this.anims.idle;this.vel.x=0;this.vel.y=0;}},update:function(){if(this.state=="defend"){this.chooseTarget();}else if(this.state=="walk"){if(this.target===null||this.target._killed||this.target.health==0){this.chooseTarget();}else{this.walkToTarget();}}else if(this.state=="attack"){if(!this.collided||this.target._killed){this.chooseTarget();}}
this.parent();}});});

// lib/game/entities/miner.js
ig.baked=true;ig.module('game.entities.miner').requires('game.entities.soldier').defines(function(){EntityMiner=EntitySoldier.extend({animSheet:new ig.AnimationSheet('media/sprites/ax.png',8,8),miner:true,health:25,enemyBox:null,init:function(x,y,settings){this.parent(x,y,settings);var boxes=ig.game.getEntitiesByType(EntityScorebox);if(boxes[0].controller==this.opponent)
this.enemyBox=boxes[0];else
this.enemyBox=boxes[1];this.chooseTarget();this.walkToTarget();},chooseTarget:function(){this.target=this.enemyBox;this.state="walk";this.currentAnim=this.anims.walk;},update:function(){if(this.state=="walk"){if(this.target===null||this.target._killed||this.target.health==0){this.chooseTarget();}else{this.walkToTarget();}}else if(this.state=="attack"){if(ig.game.score[this.opponent]==0){this.kill();}
if(!this.collided||this.target._killed){this.chooseTarget();}}
this.parent();}});});

// lib/game/entities/scorebox.js
ig.baked=true;ig.module('game.entities.scorebox').requires('impact.entity').defines(function(){EntityScorebox=ig.Entity.extend({size:{x:28,y:15},collides:ig.Entity.COLLIDES.FIXED,animSheet:new ig.AnimationSheet('media/sprites/scorebox.png',28,15),scorebox:true,health:9000,init:function(x,y,settings){this.parent(x,y,settings);this.opponent=(this.controller+1)%2;},update:function(){if(this.controller==0){var w=ig.game.font.widthForString(("0"+ig.game.score[0]).slice(-2));this.size.x=w;this.pos.x=64-w/2;}else if(this.controller==1){var w=ig.game.font.widthForString(("0"+ig.game.score[1]).slice(-2));this.size.x=w;this.pos.x=(ig.system.width-64)-w/2;}
if(this.health<=8900){this.health+=100;if(ig.game.score[this.controller]>0){ig.game.score[this.controller]--;ig.game.score[this.opponent]++;}}}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.levels.level','game.entities.playerbrick','game.entities.ball','game.entities.swordsman','game.entities.pikeman','game.entities.miner','game.entities.scorebox').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),score:[10,10],spawnTimers:[new ig.Timer(.25),new ig.Timer(.25)],gameOver:false,winner:-1,centers:[0,0],outer:[16,304],init:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.SHIFT,'shift');ig.input.bind(ig.KEY.W,'w');ig.input.bind(ig.KEY.S,'s');ig.input.bind(ig.KEY.D,'d');ig.input.bind(ig.KEY.A,'a');ig.input.bind(ig.KEY.SPACE,'space');this.loadLevel(LevelLevel);for(var i=0;i<10;i++){this.spawnEntity(EntityPlayerbrick,0,80+i*8,{input:{up:'w',down:'s'},controller:0});this.spawnEntity(EntityPlayerbrick,8,80+i*8,{input:{up:'w',down:'s'},controller:0});this.spawnEntity(EntityPlayerbrick,ig.system.width-8,80+i*8,{input:{up:'up',down:'down'},controller:1});this.spawnEntity(EntityPlayerbrick,ig.system.width-16,80+i*8,{input:{up:'up',down:'down'},controller:1});}
this.spawnEntity(EntityBall,ig.system.width/2,ig.system.height/2);this.spawnEntity(EntityScorebox,64,8,{controller:0});this.spawnEntity(EntityScorebox,ig.system.width-64,8,{controller:1});},checkGameOver:function(){if(this.score[0]>=25){this.gameOver=true;this.winner=0;return;}else if(this.score[1]>=25){this.gameOver=true;this.winner=1;return;}
var bricks=ig.game.getEntitiesByType(EntityPaddlebrick);var players=[0,0];for(var i=0;i<bricks.length;i++){players[bricks[i].controller]++;}
if(players[0]==0){this.gameOver=true;this.winner=1;}else if(players[1]==0){this.gameOver=true;this.winner=0;}},paddleCenters:function(){var bricks=ig.game.getEntitiesByType(EntityPaddlebrick);var top=[240,240];var bottom=[0,0];for(var i=0;i<bricks.length;i++){var c=bricks[i].controller;if(bricks[i].pos.y<top[c]){top[c]=bricks[i].pos.y;}
if(bricks[i].pos.y+8>bottom[c]){bottom[c]=bricks[i].pos.y+8;}}
this.centers[0]=(bottom[0]+top[0])/2;this.centers[1]=(bottom[1]+top[1])/2;},update:function(){this.checkGameOver();if(!this.gameOver){this.parent();this.paddleCenters();if(this.score[0]>0){if(ig.input.state('d')&&this.spawnTimers[0].delta()>=0){this.spawnTimers[0].reset();this.spawnEntity(EntitySwordsman,16,this.centers[0],{controller:0});this.score[0]--;}
if(ig.input.state('a')&&this.spawnTimers[0].delta()>=0){this.spawnTimers[0].reset();this.spawnEntity(EntityPikeman,16,this.centers[0],{controller:0});this.score[0]--;}
if(ig.input.state('space')&&this.spawnTimers[0].delta()>=0){this.spawnTimers[0].reset();this.spawnEntity(EntityMiner,16,this.centers[0],{controller:0});this.score[0]--;}}
if(this.score[1]>0){if(ig.input.state('left')&&this.spawnTimers[1].delta()>=0){this.spawnTimers[1].reset();this.spawnEntity(EntitySwordsman,ig.system.width-24,this.centers[1],{controller:1});this.score[1]--;}
if(ig.input.state('right')&&this.spawnTimers[1].delta()>=0){this.spawnTimers[1].reset();this.spawnEntity(EntityPikeman,ig.system.width-24,this.centers[1],{controller:1});this.score[1]--;}
if(ig.input.state('shift')&&this.spawnTimers[1].delta()>=0){this.spawnTimers[1].reset();this.spawnEntity(EntityMiner,ig.system.width-24,this.centers[1],{controller:1});this.score[1]--;}}}},draw:function(){this.parent();this.font.draw(("0"+this.score[0]).slice(-2),64,8,ig.Font.ALIGN.CENTER);this.font.draw(("0"+this.score[1]).slice(-2),ig.system.width-64,8,ig.Font.ALIGN.CENTER);if(this.gameOver){this.font.draw("Player "+this.winner+" Wins!",ig.system.width/2,ig.system.height/2,ig.Font.ALIGN.CENTER);}}});ig.main('#canvas',MyGame,60,320,240,2);});